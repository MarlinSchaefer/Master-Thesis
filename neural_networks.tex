...
To minimize this loss, the weights and biases of the different layers are changed, usually using an algorithm called \emph{gradient descent}. It works by calculating the gradient of some layer with respect to its weights and biases and changing their values by following the negative gradient. For notational simplicity we'll denote the weights and biases of a network by $\theta$ and call them collectively parameters. It is understood that $\theta=\lr{W^{1},b^1, W^2, b^2, \cdots}$. Gradient descent is then given by
...
\begin{figure}
\centering
\input{tikzgraphics/tikz_max_pooling}
...
During the evaluation process, the original paper \cite{dropout_invention} suggests reweighing the weights by the dropout probability, to get an averaging effect. This step is however not done in the software library Keras used in this work. Therefore, the dropout layer has no effect when a network that utilizes this form of regularization is used. \textcolor{red}{[Citation]}
