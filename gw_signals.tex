...
\end{equation}
The angular momentum, that is carried away, comes from the total angular momentum of the source. This in turn is comprised of the orbital angular momentum as well as the individual spins of the two component masses of a binary system. Therefore the 9 parameters of a waveform can be extended to 15, if both objects are allowed to rotate. The 6 additional parameters are the individual angular momenta of the two masses. Two further parameters influence the waveform, if the objects are not rigid and allowed to deform. This is true for binary neutron stars, as neutron stars are not singularities.\\
...
\noindent
As we will see in \autoref{sec:matched_filtering}, accurate models for the waveforms are necessary to detect them using traditional methods. This condition is not dropped for our approach of using machine learning algorithms, as they can only detect waveforms from distributions they have sampled before. \textcolor{red}{Citation?} Though the waveform derived in \eqref{def:linear_waveforms_time_evolution} gives a good qualitative overview of the rough amplitude evolution of inspiraling binary systems, it is hardly an accurate model. The main drawback of this model are the dynamics used to describe the motion of the system. It assumed \eqref{def:linear_approximation}, which in turn means, \gls{gw} and source-dynamics can be separated. Therefore the motion of binary system is dictated by Newtonian dynamics, while the \gls{gw} are a relativistic effect.\\
To overcome this issue, a different approach to simplifying and approximating the Einstein equation \eqref{def:einstein_equation} is taken. Instead of using $g_\mn\approx h_\mn +\eta_\mn$, the metric $g_\mn$ is expanded in powers of the small parameter $\epsilon\sim v/c \sim \lr{R_s / d}^{1/2}$, where $v$ is the typical speed inside the source, $R_s$ is the Schwarzschild radius attributed to the systems total mass and $d$ is the size of the source. Therefore $\epsilon$ is small, if the source is not too compact and speeds are low compared to the speed of light.
