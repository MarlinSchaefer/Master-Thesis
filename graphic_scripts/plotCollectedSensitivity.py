...
def main():
    ...
    for i, pt in enumerate(ps):
        ...
        lab = p[minIdx]
        ax.semilogx(fa[minIdx][:-1], z[minIdx][:-1], label='p-score: %.2f' % lab, color=colors[i])
    ...
    for i, pt in enumerate(snrs):
        ...
        lab = snr[minIdx]
        ax.semilogx(fa.transpose()[minIdx][:-1], z.transpose()[minIdx][:-1], label='SNR: %.2f' % lab, color=colors[i])
    ax.set_xlabel('False alarms per month')
    ax.set_ylabel('Sensitive distance in Mpc')
    x_low, x_high = ax.get_xlim()
    ax.set_xlim(x_high, 9*10**-1)
